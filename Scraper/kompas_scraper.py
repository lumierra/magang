...
def get_dataMonth(global_category=None, tahun=None, bulan=None):
    all_data = []
...
def get_dataDaily(global_category=None, tahun=None, bulan=None, tanggal=None):
    all_data = []
...
def main():
    ...
    if pilihan == 1:
        ...
        all_data = get_dataMonth(category, year, month)
        temp_data = get_content_fix((all_data))

        all_data2 = clean_data(temp_data)
        ...
        all_data = get_dataDaily(category, year, month, day)
        temp_data = get_content_fix((all_data))
        ...
        # db = Database()
        for ad in all_data2:
            db.insert_content(ad['category'], ad['title'], ad['description'], ad['url'], ad['sub_category'], ad['publishedAt'],
                              ad['img'], ad['content'], ad['status_data'])
    else:
